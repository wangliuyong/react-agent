...
| 原则 | 实现 |
|------|------|
| LangGraph 编排 | 默认 `agentRuntime=langgraph`：聊天 Supervisor 多角色 + 工作流图执行 |
...
│   │   ├── agent/
│   │   │   ├── loop.ts           # 聊天/步骤入口（委托 LangGraph 或 legacy）
│   │   │   ├── graph-bridge.ts   # stream → AgentEvent、abort/continue
│   │   │   ├── llm-langchain.ts  # ChatOpenAI（DashScope）
│   │   │   ├── llm.ts            # legacy OpenAI 流式（回滚用）
