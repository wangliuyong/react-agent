...
│       │                                                              │
│       ├── agent/loop.ts ── llm.ts (DashScope 流式)                   │
│       │        └── tools/* (ReAct 工具注册表)                         │
│       │                                                              │
│       ├── browser/service.ts (Playwright 持久化上下文)                  │
│       │        ├── xhs-publish.ts / douyin-publish.ts                │
│       │        └── human-input.ts (拟人点击/打字)                     │
│       │                                                              │
│       └── schedule/scheduler.ts (定时轮询 → 触发 Agent)               │
└──────────────────────────────────────────────────────────────────────┘
        │
        ▼
...
| 原则 | 实现 |
|------|------|
| 单一 ReAct 循环 | 模型产出 `tool_calls` → 主进程执行 → 结果回灌，直到 `done` |
| 工具即能力边界 | 新增功能优先注册工具，不改 loop 核心逻辑 |
| 无数据库 | 全部数据 JSON 落盘到 `userData`，读写通过 `store/*` |
...
├── resources/skill-templates/    # 内置技能模板
...
核心文件：`electron/main/agent/loop.ts`
...
sequenceDiagram
    ...
    UI->>Loop: postAgentChat(sessionId, content)
    Loop->>Loop: 追加 user 消息，持久化
    loop 推理轮次
        Loop->>LLM: streamChat(messages, tools)
        LLM-->>Loop: text_delta / tool_calls
        alt 有 tool_calls
            Loop->>Tools: execute(name, args, ctx)
            Tools-->>Loop: result (可能 await_user)
            Loop->>Loop: 追加 tool 消息，emit 事件
        else 纯文本结束
            Loop->>UI: emit done
        end
    end
...
`BASE_SYSTEM_PROMPT` 定义 Agent 角色与工作方式；`buildSystemPrompt()` 会追加用户启用的项目技能（`.cursor/skills/*/SKILL.md`）。

### 4.3 会话控制

| 机制 | 说明 |
|------|------|
| `abortMap` | 每会话一个 `AbortController`，`postAgentAbort` 可中止 |
| `continueWaiters` | 工具返回 `await_user` 时挂起，`postAgentContinue` 恢复 |
| `pendingToolMeta` | 本地简化存储 assistant 消息，调用 API 时还原 `tool_calls` |
| `maxTurns` | 设置页可配，防止无限工具循环 |

### 4.4 LLM 层

`electron/main/agent/llm.ts` 使用 `openai` SDK，指向百炼兼容端点，支持流式 `chat.completions`。
...
技能文件位于仓库 `.cursor/skills/<skill-id>/SKILL.md`。
...
| 模板安装 | `resources/skill-templates/` |
