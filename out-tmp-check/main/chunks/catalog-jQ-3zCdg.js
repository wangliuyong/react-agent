...
function formatSchemaParameters(parameters) {
  ...
  return entries.map(([key, schema]) => {
    ...
    const enumVals = Array.isArray(schema.enum) ? `，取值：\`${schema.enum.join("` | `")}\`` : "";
    return `- \`${key}\`（${type}，${req}${enumVals}）${desc ? ` — ${desc}` : ""}`;
  }).join("\n");
}
function queryToolUsageGuide(tool) {
...
const SKIP_FILES = /* @__PURE__ */ new Set(["index.ts", "types.ts", "catalog.ts", "langchain-adapter.ts"]);
function queryToolsSourceDir() {
  const cwdCandidate = path.join(process.cwd(), "electron/main/agent/tools");
  try {
    const { app } = require("electron");
    const appCandidate = path.join(app.getAppPath(), "electron/main/agent/tools");
    if (fs.existsSync(appCandidate)) return appCandidate;
  } catch {
  }
  return cwdCandidate;
}
function queryExtractToolSource(fileContent, toolName) {
...
function queryAgentToolsCatalog() {
  const sourceMap = queryToolSourceMap();
  let roleToolOverrides = {};
  let customRoles = [];
  try {
    const s = index.querySettings();
    roleToolOverrides = s.roleToolWhitelistOverrides ?? {};
    customRoles = s.customAgentRoles ?? [];
  } catch {
  }
  const tools = index.getAllTools().map((tool) => {
  ...
  return {
    tools,
    roleInjections: index.queryRoleToolInjections(roleToolOverrides, customRoles),
    registeredCount: tools.length
  };
}
exports.queryAgentToolsCatalog = queryAgentToolsCatalog;
