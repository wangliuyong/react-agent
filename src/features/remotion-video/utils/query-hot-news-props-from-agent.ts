...
import {
  HOT_TOPIC_SOURCE_OPTIONS,
  type HotTopicSource
} from '../constants/hot-topic-sources'
...
/** 用户所选信息来源 → 画面可用的中文数据来源文案 */
function queryHotSourceDisplayLabel(source: HotTopicSource | 'all'): string {
  return (
    HOT_TOPIC_SOURCE_OPTIONS.find((opt) => opt.value === source)?.label ?? '综合热点'
  )
}
...
/**
 * 调用 Agent：拉热点标题 → 查每条详情 → 智能决定条数与展示时长 → 输出 HotNewsProps。
...
export async function queryHotNewsPropsFromAgent(
  ...
  const prompt = [
    ...
    `2) 结合「用户素材或要求」与视频时长，智能决定：展示条数（${budget.minItems}-${budget.maxItems}）、` +
  ...
  while (Date.now() < deadline) {
    ...
    if (assistant) {
      ...
      const props = parsed
        ? queryNormalizeHotNewsProps(parsed, budget, {
