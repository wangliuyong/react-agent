...
灵犀通过大模型驱动的 LangGraph 多智能体 ReAct Agent，自动完成内容创作、多渠道发布、流程编排、AI/Remotion 视频生成与消息通知。所有数据保存在本机。
...
| 快捷任务卡       | 新会话欢迎页提供预设任务卡，一键发送常见指令（含创建定时任务/发布计划/规则）        |
| 聊天执行指令      | 在聊天中发送「执行定时任务/任务/流程 + 名称」直接触发                  |
| 聊天创建配置      | Agent 通过工具落盘定时任务、发布计划与用户规则，侧栏列表自动刷新            |
...
| 规则          | MDC 规则启用后注入各角色 system prompt；支持聊天 `post_agent_rule` 创建 |
...
在 **general** 对话中，可用自然语言让 Agent 调用下列工具（欢迎页也有「设置定时任务」「创建发布计划」「添加一条规则」快捷卡）：


| 工具                                              | 作用                            |
| ----------------------------------------------- | ----------------------------- |
| `query_scheduled_tasks` / `post_scheduled_task` | 查看 / 保存定时任务（新建默认未启用）          |
| `query_publish_plans` / `post_publish_plan`     | 查看 / 保存发布计划（normal 至少 1 个子任务） |
| `query_agent_rules` / `post_agent_rule`         | 查看 / 保存用户规则（下轮对话起注入 prompt）   |


保存后 **定时任务 / 发布 / 规则** 页面会通过主进程事件自动刷新，无需手动重开应用。
...
适合 **多渠道、多子任务串行发布** 或 **串联流程** 的场景。也可在聊天中用 `post_publish_plan` 创建计划后再到本页微调或运行。
...
1. 侧栏进入 **定时任务**，或聊天中用 `post_scheduled_task` 创建。
2. 配置重复规则、执行时间与动作类型：
  - **发布计划**：关联工作台计划
  - **自定义指令**：直接发送 Agent 指令
  - **流程**：关联工作流
...
- **规则**：管理 MDC 规则，启用后注入各角色 system prompt；也可在聊天中用 `post_agent_rule` 新增。
