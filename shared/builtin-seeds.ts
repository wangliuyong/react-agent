      ...
      subTasks: [
        {
          ...
          autoPublish: true,
          contentPrompt:
            '内容主题：搜罗昨日 ai 最新热门新闻。热点：优先 fetch_hot_topics（source 依次 tophub、weibo、douyin、baidu）。配图：从相关新闻来源网页用 fetch_web_images 抓取封面图（本地上传可选）。未登录时会暂停等人扫码。'
        },
        {
...
export function createBuiltinScheduledTasks(now = Date.now()): ScheduledTask[] {
  return [
    ...
    },
    {
      ...
      customPrompt:
        '请调研上周人工智能与科技行业的热点事件（可调用 fetch_hot_topics，优先 source=tophub，不足再试 weibo/baidu/douyin），整理成 5 条要点摘要，每条包含标题、一句话说明和参考来源链接。输出 Markdown 格式，便于后续改写为发布内容。',
    ...
    },
    {
      ...
      actionType: 'custom_prompt',
      customPrompt:
        '请获取昨日热搜中与人工智能、科技、互联网相关的热点（优先调用 fetch_hot_topics，source 依次尝试 tophub、weibo、baidu、douyin）。' +
